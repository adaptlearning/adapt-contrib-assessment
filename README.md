...
>>**\_resetIncorrectOnly** (boolean): If set to `true`, only blocks where every question was not answered correctly will be reset when the assessment is retried. Blocks containing at least one correctly answered question will not be reset and will not be presented again. Note: if a block contains multiple questions and at least one is correct, the entire block — including any questions not answered correctly within it — will be excluded from the retry. For most assessments, where each block contains a single question, this means only questions not answered correctly are retried. Acceptable values are `true` or `false`. Default: `false`.
